...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, clip_state_dict, T, transf_layers=6):
        super().__init__()
        ...
        if self.sim_header == "Transf" :
            self.transformer = TemporalTransformer(width=embed_dim, layers=transf_layers, heads=transformer_heads)
            print('layer=', transf_layers)
